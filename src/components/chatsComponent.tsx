...
export const ChatsComponent = () => {
  ...
  const getUserChats = async () => {
    ...
    const q = query(chatsRef, where('owner', '==', currentUser.uid));
  ...
  const handleSearchChats = (query) => {
    // це треба додати в input на подію onChange
  ...
  return (
    <section className="flex gap-6 h-full">
      <div
        className={`bg-background-second w-full p-[15px] rounded-xl h-full ${role?.role === 'coach' || role?.role === 'moderator' ? 'max-w-[370px]' : 'max-w-[256px]'}`}
      >
        ...
        <ul>
          {allUsersChats.length > 0 ? (
            allUsersChats.map((chat) => (
              <li key={chat.id} className="flex items-center h-[42px] truncate">
                <button
                ...
                {chat.id === activeChatId && (
                  <div className="bg-primary w-5 h-5 rounded-full p-2" />
                )}
                {activeChatPlusId === chat.id &&
                  (role?.role === 'coach' || role?.role === 'moderator') && (
      ...
      {activeChatId ? (
        ...
        <div className="flex flex-col justify-center items-center w-full gap-[26px]">
          <Image src={message} alt="message icon" width={56} />
          <p className="font-bold text-[32px]">Chat not selected</p>
          <p className="font-medium text-[24px] text-neutral2 text-center">
            Select the chat to see the message
          </p>
        </div>
      )}
    </section>
