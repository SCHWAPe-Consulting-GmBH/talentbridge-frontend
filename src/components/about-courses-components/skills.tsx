  ...
  return (
    <section className="mb-[100px]">
      ...
      {Object.keys(data).map((key) => (
        <div key={uuidv4()} className="flex justify-between relative mb-8 after:block after:h-[2px] after:bg-primary after:opacity-[30%]  after:absolute after:top-[-16px] after:w-full">
          <p className="text-themetext font-extrabold text-[48px]">{key}</p>
          <ol className="list-disc list-inside pl-4">
            {data[key].map(item => (
              <li key={uuidv4()} className="text-themetext font-medium text-[24px] mb-4">{item}</li>
            ))}
          </ol>
        </div>
      ))}
